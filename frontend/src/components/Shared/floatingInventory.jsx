...
const FloatingComponent = ({ onClose, productData, setSelectedProduct, selectedProduct, products, selected, setSelected }) => {
  ...
  return (
    <div
      className={`fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-10 transition-opacity duration-300 ${
        isVisible ? "opacity-100" : "opacity-0"
      }`}
    >
      <div
        className={`bg-white flex flex-col p-8 pb-4 rounded-lg shadow-lg w-[50rem] h-[35rem] relative transition-transform duration-300 transform ${
      ...
      >
        ...
        <div className="flex justify-start items-center mb-6 gap-8">
          ...
          <input
            ...
            placeholder="Search (name, size/model, type)"
        ...
        <div className="p-2 grid grid-cols-4 gap-4 overflow-auto h-[24rem] shrink-0 scrollbar-thin">
          {filteredData.map((item) => (
            <div
              key={item.prod_id}
              className={`relative bg-white cursor-pointer border-[2px] ${
                ...
                selected?.prod_id === item.prod_id
                  ...
                  : ""
              } w-40 h-44 rounded-lg transform hover:scale-105 transition-transform duration-300`}
              onClick={() => {
            ...
            >
              <img
                ...
                src={item.image ? `http://localhost:4000${item.image}` : ""}
                alt="no image"
              />
              <div
                className={`absolute text-[#e6ebf7] bottom-0 bg-[#47505f] w-full px-4 py-2 rounded-b-md ${
                  isProductAdded(item.prod_id) ? "" : ""
                }`}
              >
